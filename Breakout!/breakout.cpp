...
int main(int argc, char *argv[]) {
  ...
  if (argc == 3) {
    try {
      ...
      if (inputFPS > 60 || inputFPS < 10 || inputBallSpeed > 10 ||
          inputBallSpeed < 1) {
        throw 1;
      }
    ...
    cout << "Your game will start with a preset FPS of 60 and enjoyable "
            "ballspeed. Have fun!!!"
         << endl;
  } else {
    handleInvalidCmdArgs();
  ...
  window =
      XCreateSimpleWindow(display, DefaultRootWindow(display), 10, 10,
                          windowWidth, windowHeight, 2, foreground, background);
  ...
  string msg1 =
      "This game is created by Adithya Venkatarao (ID: a32venka && Student#: "
      "20606942)";
  string msg2 =
      "To play this game, use the 'a' and 'd' buttons to move the paddle left "
      "and right respectively.";
  string msg3 =
      "You can press 'q' at anytime to quit the game. Press 'r' to start "
      "playing!!!";
  ...
  while (true) {
    ...
    if (end - lastRepaint > 1000000 / FPS) {
      ...
      if (!gameInPlay) {
        ...
        // clear background
        XFillRectangle(display, buffer, gcWhite, 0, 0, windowWidth,
                       windowHeight);
      ...
      if (ball.xPos() + ball.size() / 2 > windowWidth ||
        ...
        string endGameText = "You have lost with a total score of " +
      ...
      bool ballLeftOfPaddleNow =
          ball.xPos() - ball.size() / 2 < paddle.xPos() + paddle.width();
      bool ballRightOfPaddleNow = ball.xPos() + ball.size() / 2 > paddle.xPos();
      bool ballAbovePaddleNow =
          ball.yPos() - ball.size() / 2 < paddle.yPos() + paddle.height();
      bool ballBelowPaddleNow = ball.yPos() + ball.size() / 2 > paddle.yPos();

      if (ballLeftOfPaddleNow && ballRightOfPaddleNow && ballAbovePaddleNow &&
          ballBelowPaddleNow) {
        if (!ballLeftOfPaddle || !ballRightOfPaddle) {
          ball.invertXDir();
        } else if (!ballBelowPaddle) {  // don't interact with hit detection
                                        // from below paddle
          ball.invertYDir();

          // hit ball to the left if it hits the left half of the paddle, else
          // hit it to the right
      ...
      for (auto &blockinfo : blocks) {
        ...
        bool ballLeftOfBlockNow =
            ball.xPos() - ball.size() / 2 < block->xPos() + block->width();
        bool ballRightOfBlockNow =
            ball.xPos() + ball.size() / 2 > block->xPos();
        bool ballAboveBlockNow =
            ball.yPos() - ball.size() / 2 < block->yPos() + block->height();
        bool ballBelowBlockNow = ball.yPos() + ball.size() / 2 > block->yPos();

        if (ballLeftOfBlockNow && ballRightOfBlockNow && ballAboveBlockNow &&
            ballBelowBlockNow) {
      ...
      XCopyArea(display, buffer, window, gcBlack, 0, 0, windowWidth,
                windowHeight,  // region of pixmap to copy
